  ...
  return (
    <section id="layanan" className="py-20 lg:py-28 relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-gradient-to-b from-slate-100 via-[#070b24] to-slate-100 border border-slate-200/80 rounded-3xl p-6 sm:p-10 shadow-2xl shadow-cyan-950/20 mb-16 relative overflow-hidden">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 lg:gap-12 items-center relative z-10">
            ...
            <div className="lg:col-span-5 space-y-4 text-left">
              ...
              <p className="text-slate-600 text-xs sm:text-sm lg:text-[15px] leading-relaxed font-sans">
                {lang === 'id'
                  ? 'Kami menyediakan berbagai solusi teknologi terintegrasi yang mencakup sistem jaringan, keamanan, komunikasi, serta dukungan Mechanical & Electrical untuk memenuhi kebutuhan operasional modern.'
            ...
            <button
              ...
              className={`px-4 py-2 text-xs sm:text-sm font-semibold rounded-xl transition-all cursor-pointer ${
                activeCategory === cat.id
                  ...
                  : 'bg-white/85 text-slate-500 hover:text-white hover:bg-slate-100 border border-slate-200'
              }`}
            >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredServices.map((service) => (
            <div
              ...
              className="bg-white/85 border border-slate-200/90 hover:border-cyan-500/40 rounded-2xl p-6 flex flex-col justify-between transition-all duration-300 hover:shadow-xl hover:shadow-cyan-950/30 group hover:-translate-y-1 relative"
            >
              {/* Card Top */}
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div className="p-3 rounded-xl bg-white border border-slate-200 text-cyan-600 group-hover:bg-cyan-500/10 group-hover:border-cyan-500/30 group-hover:text-cyan-700 transition-all">
                    {getServiceIcon(service.iconName)}
                  </div>
                  <span className="text-[11px] font-mono text-slate-500 group-hover:text-cyan-600 transition-colors">
                    #0{SERVICES.findIndex(s => s.id === service.id) + 1}
                  </span>
                </div>

                <h3 className="text-lg sm:text-xl font-bold text-white mb-2 font-display group-hover:text-cyan-700 transition-colors">
                ...
                <div className="space-y-1.5 pt-2 border-t border-slate-200/80 mb-4">
                  ...
                  {service.features.length > 3 && (
                    <div className="text-[11px] text-cyan-600/80 pl-5">
                ...
                {service.brands && service.brands.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mb-5">
                    {service.brands.slice(0, 4).map((brand, bIdx) => (
                      <span
                        key={bIdx}
                        className="text-[10px] px-2 py-0.5 rounded bg-white border border-slate-200 text-slate-500"
                      >
                        {brand}
                      </span>
                    ))}
              ...
              <div className="pt-3 border-t border-slate-200 flex items-center justify-between gap-2">
                <button
                  ...
                  className="text-xs font-semibold text-cyan-600 hover:text-cyan-700 flex items-center gap-1 cursor-pointer transition-colors"
                >
                ...
                <button
                  ...
                  className="px-3 py-1.5 text-xs font-semibold rounded-lg bg-slate-100 hover:bg-slate-200 text-slate-800 border border-slate-200 hover:border-cyan-500/40 transition-colors cursor-pointer"
                >
        ...
        <div className="mt-14 bg-gradient-to-r from-slate-100 via-white/95 to-cyan-950/40 border border-cyan-500/30 rounded-2xl p-6 sm:p-8 flex flex-col md:flex-row items-center justify-between gap-6">
          ...
          <a
            ...
            className="flex-shrink-0 inline-flex items-center gap-2 px-5 py-3 rounded-xl font-bold text-xs sm:text-sm text-slate-950 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-600 shadow-lg shadow-cyan-500/20"
          >
