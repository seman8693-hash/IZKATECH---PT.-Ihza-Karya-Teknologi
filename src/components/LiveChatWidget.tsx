...
export const LiveChatWidget: React.FC<LiveChatWidgetProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed bottom-20 right-4 sm:right-6 z-50 w-[calc(100vw-2rem)] sm:w-96 shadow-2xl transition-all duration-200">
      <div className="bg-white/95 border border-cyan-500/30 rounded-2xl overflow-hidden backdrop-blur-xl flex flex-col shadow-cyan-950/40">
        
        {/* Chat Header */}
        <div className="bg-gradient-to-r from-slate-100 via-slate-800 to-cyan-950 p-3.5 border-b border-cyan-500/20 flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="relative">
              <div className="w-8 h-8 rounded-full bg-cyan-500/20 border border-cyan-400/40 flex items-center justify-center text-cyan-700">
                <Headphones className="w-4 h-4" />
              </div>
              <span className="absolute -bottom-0.5 -right-0.5 w-2.5 h-2.5 rounded-full bg-emerald-400 border-2 border-slate-300"></span>
            </div>
            <div>
              <div className="flex items-center gap-1.5">
                <span className="text-xs font-bold text-white tracking-wide">Live Support CRM</span>
                <span className="px-1.5 py-0.2 rounded text-[9px] font-mono bg-cyan-500/20 text-cyan-700 border border-cyan-500/30">
                  ONLINE
                </span>
              </div>
              <p className="text-[10px] text-slate-500">Tim Solution Architect PT. Ihza Karya Teknologi</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1 text-slate-500">
            <button 
              ...
              className="p-1 hover:text-white rounded hover:bg-slate-100 transition-colors cursor-pointer"
          ...
          <>
            ...
            {!hasStarted ? (
              <form onSubmit={handleStartChat} className="p-4 space-y-3 text-xs">
                <div className="p-3 rounded-xl bg-cyan-950/40 border border-cyan-500/20 text-slate-600 leading-relaxed">
                  <span className="font-semibold text-cyan-700 block mb-1">💬 Konsultasi Langsung dengan Tim Ahli</span>
                  Silakan isi profil singkat Anda untuk terhubung ke CRM &amp; Engineer kami secara real-time.
                </div>

                <div>
                  <label className="block text-slate-600 mb-1 font-semibold">Nama Lengkap / PIC *</label>
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-9 pr-3 py-2 rounded-xl bg-white border border-slate-200 text-white placeholder:text-slate-600 focus:outline-none focus:border-cyan-400"
                    />
                  </div>
                </div>
                ...
                <div>
                  <label className="block text-slate-600 mb-1 font-semibold">Nomor WhatsApp / Telepon</label>
                  <div className="relative">
                ...
                <div>
                  <label className="block text-slate-600 mb-1 font-semibold">Pilih Layanan yang Diminati</label>
                  <select
                    value={selectedService}
                    onChange={(e) => setSelectedService(e.target.value)}
                    className="w-full px-3 py-2 rounded-xl bg-white border border-slate-200 text-white focus:outline-none focus:border-cyan-400"
                  >
                    {SERVICES.map((s) => (
                ...
                <button
                  type="submit"
                  className="w-full py-2.5 px-4 rounded-xl font-bold text-slate-950 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 shadow-lg shadow-cyan-500/20 transition-all cursor-pointer mt-2"
                >
              ...
              <>
                {/* Messages Body */}
                <div className="p-4 h-72 sm:h-80 overflow-y-auto space-y-3 text-xs bg-white/60">
                    ...
                    if (isSystem) {
                      return (
                        <div key={msg.id} className="text-center my-2">
                          <span className="inline-block px-3 py-1 rounded-full bg-cyan-950/50 border border-cyan-500/20 text-[10px] text-cyan-700">
                            {msg.text}
                          </span>
                        </div>
                      );
                        ...
                        <div
                          className={`max-w-[85%] p-3 rounded-2xl leading-relaxed whitespace-pre-wrap ${
                            isVisitor
                              ...
                              : 'bg-slate-100/90 text-slate-800 border border-slate-200/80 rounded-tl-xs shadow-md'
                          }`}
                        >
                          {msg.text}
                        </div>
                ...
                <form onSubmit={handleSendMessage} className="p-3 border-t border-slate-200 bg-white flex items-center gap-2">
                  <input
                    ...
                    className="flex-1 px-3 py-2 rounded-xl bg-white border border-slate-200 text-white text-xs placeholder:text-slate-500 focus:outline-none focus:border-cyan-400"
                  />
                  <button
                    type="submit"
            ...
            {/* Micro footer */}
            <div className="px-3 py-1.5 bg-white/90 border-t border-slate-200/80 flex items-center justify-between text-[10px] text-slate-500 font-mono">
