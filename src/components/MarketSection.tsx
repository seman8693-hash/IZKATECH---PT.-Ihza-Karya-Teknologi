...
export const MarketSection: React.FC<MarketSectionProps> = ({ lang }) => {
  ...
  return (
    <section id="keunggulan" className="py-20 lg:py-28 bg-slate-900/30 border-t border-slate-800/80 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-16">
          {/* Industry Overview */}
          <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 rounded-2xl p-6 sm:p-8 relative">
            ...
            <p className="text-slate-300 text-sm sm:text-base leading-relaxed text-justify sm:text-left">
              {COMPANY_INFO.industryOverview}
            </p>
            <div className="mt-5 pt-4 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
              <span className="text-cyan-400 font-semibold">Transformasi Digital Berkelanjutan</span>
          ...
          <div className="bg-gradient-to-br from-slate-900 to-slate-950 border border-slate-800 rounded-2xl p-6 sm:p-8 relative">
            ...
            <div className="mt-5 pt-4 border-t border-slate-800/80 flex items-center justify-between text-xs text-slate-400">
              <span className="text-sky-400 font-semibold">Sinergi ICT + Dukungan ME</span>
        ...
        <div className="mb-20">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            {COMPANY_INFO.keyPoints.map((kp, idx) => (
              <div 
                key={idx}
                className="bg-slate-950/80 border border-slate-800 hover:border-cyan-500/40 rounded-xl p-5 flex flex-col justify-between transition-all group"
              >
                <div>
                  <div className="text-2xl font-extrabold text-slate-700 group-hover:text-cyan-400 transition-colors font-mono mb-2">
                    0{idx + 1}
                  </div>
                  <h4 className="text-sm font-bold text-white mb-2 leading-snug group-hover:text-cyan-300 transition-colors">
                    {kp.title}
                  </h4>
                ...
                <div className="mt-4 pt-2 border-t border-slate-900 flex items-center gap-1.5 text-[11px] text-cyan-400 font-semibold">
        ...
        <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {TARGET_MARKETS.map((tm, idx) => (
              <div
                key={idx}
                className="bg-slate-900/60 border border-slate-800/80 hover:border-slate-700 rounded-2xl p-6 transition-all"
              >
                <div className="p-3 w-fit rounded-xl bg-slate-950 border border-slate-800 mb-4">
                ...
                <div className="text-xs text-slate-500 pt-3 border-t border-slate-800/60">
                  <span className="font-semibold text-cyan-400/90">Contoh Proyek:</span> {tm.examples}
                </div>
              </div>
            ))}
          </div>
