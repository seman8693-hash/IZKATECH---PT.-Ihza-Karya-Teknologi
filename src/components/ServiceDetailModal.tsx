...
export const ServiceDetailModal: React.FC<ServiceDetailModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-white/90 backdrop-blur-md animate-fadeIn">
      <div 
        className="relative w-full max-w-2xl bg-white border border-slate-200/80 rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Modal Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white/75">
        ...
        <div className="p-6 overflow-y-auto space-y-6 text-left">
          ...
          {/* Scope & Key Features */}
          <div className="bg-white/75 border border-slate-200 rounded-xl p-4 space-y-3">
            <h4 className="text-xs font-bold uppercase tracking-wider text-cyan-600 flex items-center gap-2">
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {service.brands.map((brand, bIdx) => (
                  <span
                    key={bIdx}
                    className="px-3 py-1 text-xs font-medium rounded-lg bg-slate-100/90 text-cyan-700 border border-slate-200/80"
                  >
                    {brand}
                  </span>
                ))}
          ...
          <div className="p-3.5 rounded-xl bg-gradient-to-r from-cyan-950/40 to-blue-950/40 border border-cyan-500/20 text-xs text-slate-600 flex items-center justify-between">
            <span>Garansi Resmi Prinsipal &amp; Pengerjaan Standard Enterprise</span>
            <span className="font-mono text-cyan-600 font-bold">PT. IHZA KARYA TEKNOLOGI</span>
          </div>
        </div>

        {/* Modal Footer Actions */}
        <div className="px-6 py-4 border-t border-slate-200 bg-white/90 flex flex-wrap items-center justify-end gap-3">
          <button
            onClick={onClose}
          ...
          <button
            ...
            className="px-4 py-2 text-xs font-semibold text-slate-800 bg-slate-100 hover:bg-slate-200 border border-slate-600 rounded-lg transition-colors cursor-pointer"
          >
          ...
          <a
            ...
            className="inline-flex items-center gap-2 px-4 py-2 text-xs font-bold text-slate-950 bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-600 rounded-lg shadow-md shadow-cyan-500/20"
          >
